...
def index():
    ...
    # tasks = [{"task": "siivousta"}, {"task": "lisää siivousta"}]
    return render_template(
        "index.html", all_tasks=tasks
    )  # hakee oletuksena templates-kansiosta
...
def add_task():
    task = request.form[
        "task"
    ]  # haetaan index.html -sivun formista task-nimisen kentän teksti muuttujaan
    new_id = fetch_new_id(coll)
    coll.insert_one({"id": new_id, "task": task, "isComplete": False})
